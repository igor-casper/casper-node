...
mod utils;
use asertions::{
    AliceExpectBalanceChange, AliceTotalMeetsAvailable, BobExpectBalanceChange,
    CharlieExpectBalanceChange, ExecResultCost, TotalSupplyChange, TransactionFailure,
...
use crate::{
    reactor::main_reactor::tests::{
        transactions::{invalid_wasm_txn, MIN_GAS_PRICE},
        ONE_MIN,
    },
...
async fn should_accept_transfer_without_id() {
    ...
    let charlie_secret_key = test_scenario.charlie_secret_key();
    let alice_secret_key = test_scenario.alice_secret_key();
...
async fn should_native_transfer_nofee_norefund_fixed() {
    ...
    test_scenario
        ...
        .await;
    test_scenario
        .assert(AliceExpectBalanceChange::new(
    ...
    test_scenario
        .assert(CharlieExpectBalanceChange::new(
            TRANSFER_AMOUNT as i64,
            TRANSFER_AMOUNT as i64,
        ))
        .await;
    ...
    test_scenario
        ...
        .await
        .unwrap();
    test_scenario.assert(AliceTotalMeetsAvailable::new()).await;
}
...
async fn erroneous_native_transfer_nofee_norefund_fixed() {
    ...
    let charlie_secret_key = test_scenario.charlie_secret_key();
    let alice_secret_key = test_scenario.alice_secret_key();
    let bob_secret_key = test_scenario.bob_secret_key();
    let chain_name = test_scenario.chain_name();
    test_scenario.setup().await.unwrap();
    ...
    let mut txn: Transaction = Transaction::from(
        ...
        .unwrap()
        .with_initiator_addr(PublicKey::from(charlie_secret_key.as_ref()))
        .with_pricing_mode(PricingMode::Fixed {
            gas_price_tolerance: 1,
            additional_computation_factor: 0,
        })
...
async fn should_cancel_refund_for_erroneous_wasm() {
    ...
    test_scenario
        .assert(BobExpectBalanceChange::new(
...
async fn should_not_refund_erroneous_wasm_burn_fixed() {
    ...
    test_scenario
        ...
        .await;
    // Supply shouldn't change (refund handling is burn, but the wasm was erroneous so we don't calulate refund)
    test_scenario
