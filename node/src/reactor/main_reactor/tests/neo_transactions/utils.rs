...
use casper_types::{
    bytesrepr::Bytes, testing::TestRng, EraId, ExecutionInfo, FeeHandling, PricingHandling,
    PricingMode, PublicKey, RefundHandling, SecretKey, TimeDiff, Transaction, TransactionHash,
    TransactionRuntimeParams, U512,
};
...
use crate::{
    reactor::main_reactor::tests::{
        ...
        fixture::TestFixture,
        transactions::{get_balance, BalanceAmount},
...
pub(crate) struct TestStateSnapshot {
    ...
    pub(crate) alice_balance: BalanceAmount,
    pub(crate) bob_balance: BalanceAmount,
    pub(crate) charlie_balance: BalanceAmount,
...
struct ScenarioDataInstance {
    fixture: TestFixture,
    alice_key: Arc<SecretKey>,
    bob_key: Arc<SecretKey>,
    //Example key 1 that was not in genesis and is not a validator
    charlie_key: Arc<SecretKey>,
    block_height: u64,
}
...
impl TestScenario {
    ...
    pub(crate) fn alice_secret_key(&self) -> Arc<SecretKey> {
        self.data.alice_key.clone()
    }

    pub(crate) fn bob_secret_key(&self) -> Arc<SecretKey> {
        self.data.bob_key.clone()
    }

    pub(crate) fn charlie_secret_key(&self) -> Arc<SecretKey> {
        self.data.charlie_key.clone()
    }

    async fn get_state_at_height(&self, block_height: u64) -> TestStateSnapshot {
        let alice_balance = self
            .get_balance_amount(self.alice_secret_key(), block_height)
            .await;
        let bob_balance = self
            .get_balance_amount(self.bob_secret_key(), block_height)
            .await;
        let charlie_balance = self
            .get_balance_amount(self.charlie_secret_key(), block_height)
            .await;
    ...
    async fn get_balance_amount(&self, key: Arc<SecretKey>, block_height: u64) -> BalanceAmount {
        let block_height = Some(block_height);
        let public_key = PublicKey::from(&*key);

        let total = get_balance(&self.data.fixture, &public_key, block_height, true)
            .total_balance()
            .copied()
            .unwrap_or(U512::zero());
        let available = get_balance(&self.data.fixture, &public_key, block_height, false)
            .available_balance()
            .copied()
            .unwrap_or(U512::zero());
...
impl TestScenarioBuilder {
    ...
    pub async fn build(self, rng: &mut TestRng) -> TestScenario {
        ...
        let (secret_keys, stakes) = maybe_stakes_setup.unwrap_or({
            let alice_secret_key = Arc::new(SecretKey::random(rng));
            let bob_secret_key = Arc::new(SecretKey::random(rng));
        ...
        let alice_key = fixture
            .node_contexts
            .first()
            .map(|el| el.secret_key.clone())
            .unwrap()
            .clone();
        let bob_key = fixture
            .node_contexts
            ...
            .unwrap()
            .clone();
        let charlie_key = Arc::new(SecretKey::random(rng));
